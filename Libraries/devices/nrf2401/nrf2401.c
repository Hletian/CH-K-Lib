...
void NRF2401_SetRXMode(void)
{
  ...
  GPIO_SetBits((GPIO_Type*) gpNRF2401->CE_GPIO_Instance, gpNRF2401->CE_GPIO_Pin); //CE为高,进入接收模式 
}			
//该函数初始化NRF24L01到TX模式
...
uint8_t NRF2401_SendData(uint8_t *txbuf)
{
  ...
  GPIO_ResetBits((GPIO_Type*) gpNRF2401->CE_GPIO_Instance, gpNRF2401->CE_GPIO_Pin); 
	NRF2401_WriteBuffer(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  32个字节
 	GPIO_SetBits((GPIO_Type*) gpNRF2401->CE_GPIO_Instance, gpNRF2401->CE_GPIO_Pin); //启动发送	  
	while(GPIO_ReadInputDataBit((GPIO_Type*) gpNRF2401->IRQ_GPIO_Instance, gpNRF2401->IRQ_GPIO_Pin) != 0);//等待发送完成
