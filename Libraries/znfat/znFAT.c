...
UINT8 Analyse_FDI(struct FileInfo *pfi,struct FDI *pFDI)
{
 ...
 pfi->File_CDate.year=(UINT8)((temp&DATE_YEAR_MARK)+DATE_YEAR_BASE); //创建日期的年位（加上年份基数）
...
UINT8 oemstr2unistr(INT8 *oemstr,UINT16 *unistr)
{
 ...
 for(i=0;i<len;i++)
 {
  ...
  {
   res=OEM2UNI((UINT16)((((UINT16)(oemstr[i]&0X00FF))<<8)|((UINT16)oemstr[i+1]&0X00FF)),unistr+pos);
   if(res) 
   {
	unistr[0]=0;
