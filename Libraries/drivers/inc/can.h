...
typedef struct can_dev
{
	uint16_t txid; //发送地址
	uint16_t rxid; //接收地址
	uint16_t txmsgboxindex; //发送时候使用的邮箱
	uint16_t rxmsgboxindex; //接收时候使用的邮箱
	CAN_BAUDRATE_SELECT baudrate; //速度选择
	uint8_t  len; //发送每个帧数据长度 0-8
	uint8_t reclen;//接收到的数据帧长度
}CAN_DevType_t;
extern CAN_DevType_t candev; //实体化can 控制结构
